...
/*      ******************************************************************
	...
	******************************************** software.stoitsov.com  */
	...
	function bbfontstyle(bbopen, bbclose) { if ((clientVer >= 4) && is_ie && is_win) { theSelection = document.selection.createRange().text; if (!theSelection) { document.post.message.value += bbopen + bbclose; document.post.message.focus(); return; } document.selection.createRange().text = bbopen + theSelection + bbclose; document.post.message.focus(); return; } else { document.post.message.value += bbopen + bbclose; document.post.message.focus(); return; } storeCaret(document.post.message); }
        function bbstyle(bbnumber) { donotinsert = false; theSelection = false; bblast = 0; if (bbnumber == -1) { while (bbcode[0]) { butnumber = arraypop(bbcode) - 1; document.post.message.value += bbtags[butnumber + 1]; buttext = eval('document.post.addbbcode' + butnumber + '.value'); eval('document.post.addbbcode' + butnumber + '.value =\"' + buttext.substr(0,(buttext.length - 1)) + '\"'); } imageTag = false; document.post.message.focus(); return; }
  ...
  //--> </script>";
  $ResultHtml.=puHeading($language['Add']." ".$Easy["Articles"],1)."".$language['Registered users can add, edit or delete']." ".$Easy["Articles"].".<br><br>
  <b>".$language['Preview']."</b><br>
  <div style='background-color: ".$Easy["LightColor2"].";'>
  <span class=h1s>".$puHeading."</span><br><br>".BBCode2HTML(puHackers($message))."<br><b>".$language['Topic'].":</b> ".$FREET[$PageSection]." :. <b>".$language['Author'].":</b> ".puShowAuthor($Stoitsov["ID"])." :. <b>".$language['Publish date'].":</b> ".$puDate."
  </div> ".puElement("form",$EDP_SELF."&home=".$home,"post","POST").puElement("submit",$language['Preview'],"f_button","Preview")." ".puElement("submit",$language['Add'],"f_button")."<br><br>
	<div align=center>
    <table  border='0' cellspacing='1' cellpadding='2' bgcolor='".$Easy[$PageSection]."'>
    <tr><td>&nbsp;<font color=".$Easy["Background"]."><b>.: ".$language['Add']."</b></font></td> </tr>
    <tr><td bgcolor=".$Easy["Background"]."><b>".$language['Heading']."</b><br>".puElement("text","puHeading",$puHeading,400)."</td> </tr>
    <tr><td bgcolor=".$Easy["Background"]."><b>".$language['Topic'].": ".$FREET[$PageSection]."</b></td> </tr>
    <tr><td bgcolor=".$Easy["Background"]."><b>".$language['Body']."</b><br>";
			$ResultHtml.="
      <table   border=\"0\" cellspacing=\"0\" cellpadding=\"2\">
          ...
          <tr> <td colspan=\"9\"><textarea name=\"message\" rows=\"15\" cols=\"35\" wrap=\"virtual\" style=\"width:400px\" tabindex=\"3\" class=\"post\" onselect=\"storeCaret(this);\" onclick=\"storeCaret(this);\" onkeyup=\"storeCaret(this);\" class=f_text>".puHackers($message)."</textarea></td> </tr>
			</table>";
$ResultHtml.="</td>
    </tr> <tr><td bgcolor=".$Easy["Background"]."><b>".$language['Date']."</b> [YYYY-MM-DD]<br>".puElement("text","puDate",((isset($puDate) && $puDate!="") ? $puDate : date("Y-m-d") ),200)."</td> </tr>
    <tr> <td align=right>".puElement("submit",$language['Preview'],"f_button","Preview")." ".puElement("submit",$language['Add'],"f_button")."</td> </tr> ".puElement("hidden","action","add_news").puElement("hidden","puUserID",$Stoitsov["ID"]).puElement()."
  </table><br>". $Error ."</div> ";
?>
