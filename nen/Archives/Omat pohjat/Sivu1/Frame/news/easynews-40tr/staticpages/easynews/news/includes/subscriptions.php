...
if ($_POST['save'] == 'save changes') {
        ...
        if ($opmlfilename) {
            ...
            if ($opmlfilename) {
                ...
                for($i = 0;$i < $totalpos;$i++) {
                    ...
                    $sfwrite.="\t\t<outline type=\"rss\" position=\"" . $temppos . "\" text=\"" . specialchars($temptitle) . "\" title=\"" . specialchars($temptitle) . "\" description=\"" . specialchars($tempdesc) . "\" xmlUrl=\"" . specialchars($tempxmlurl) . "\" htmlUrl=\"" . specialchars($temphtmlurl) . "\" refreshTime=\"" . $temprefresh . "\" showedItems=\"" . $tempshowed . "\" isSubscribed=\"" . $tempsub . "\" language=\"" . $templang . "\" />\n";
                }
                $sfwrite.="\t</body>\n</opml>";
                echo "Subscription list <b>".$crtcategory."</b> modified.";
                mysql_query("UPDATE edp_newsopml SET opmlbody='".$sfwrite."'  WHERE opmlname='".$crtcategory."';");
            } else {
    ...
    if (strstr($opmldata, "Error parsing subscriptions file") == false && strstr($opmldata, "Error opening subscriptions file") == false) {
        ...
        if ($opmlfilename) {
            ...
            $sfwrite.="\t</body>\n</opml>";
            echo "Deleted selected items from subscription list (<b>".$crtcategory."</b>).";
            mysql_query("UPDATE edp_newsopml SET opmlbody='".$sfwrite."'  WHERE opmlname='".$crtcategory."';");
        } else {
            echo "Error opening the subscription list for writing !";
        }
