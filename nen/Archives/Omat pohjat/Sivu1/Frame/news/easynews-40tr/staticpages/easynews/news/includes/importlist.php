...
function parse_iopmlfile1($filename, $opmlok = false)
{
    global $items, $itemcount;
    $items = array();
    $itemcount = 0;
		...
		<tr align="center" valign="top" bgcolor="#F3F3F3">
		  ...
		  <td>
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
			  <tr>
                                <td width="402"><font color="#000066" size="2" face="Verdana, Arial, Helvetica, sans-serif"><a href="javascript:open('{htmlurl}')" onclick="window.open('{htmlurl}'); return false;"> &laquo;&raquo;  {chantitle}</a></font></td>
			  ...
			  <tr>
				...
				HTML URL : <i>{htmlurl}</i>
                                ...
                                <td align="right"><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><a href="javascript:open('{xmlurl}')" onclick="window.open('{xmlurl}'); return false;">XML</a></font></td>
			  </tr>
			  <tr>
...
<?php } elseif ($_POST['save'] == 'add selected feeds') {
     $opml_result = parse_iopmlfile1($opmlfilename, true);
     if ($opml_result == 'opmlok') {
        set_magic_quotes_runtime(0);
            ...
            $sfwrite.="<?xml version=\"1.0\"?>\n<!-- subscription list generated by zFeeder " . ZF_VER . " on " . gmdate("D, d M Y H:i:s \G\M\T") . " -->\n";
            $sfwrite.="<opml version=\"1.0\">\n\t<head>\n\t\t<title>" . $crtcategory . "</title>" . $dateModified . $ownername . $owneremail . "\t</head>\n";
            $sfwrite.="\t<body>\n";
            $lastpos = 0;
            foreach($items as $i => $item) {
                if ($item['position'] > $lastpos) $lastpos = $item['position'];
                $sfwrite.="\t\t<outline type=\"rss\" position=\"" . $item['position'] . "\" text=\"" . specialchars($item['title']) . "\" title=\"" . specialchars($item['title']) . "\" description=\"" . specialchars($item['description']) . "\" xmlUrl=\"" . specialchars($item['xmlurl']) . "\" htmlUrl=\"" . specialchars($item['htmlurl']) . "\" refreshTime=\"" . $item['refreshtime'] . "\" showedItems=\"" . $item['showeditems'] . "\" isSubscribed=\"" . $item['issubscribed'] . "\" language=\"" . $item['language'] . "\" />\n";
            }
            ...
            foreach($_POST as $key => $value) {
                if ($key == "xmlurl" . $i && $value != '' && $_POST["addbox" . $i] == 'checkbox') {
                    ...
                    $sfwrite.="\t\t<outline type=\"rss\" position=\"" . $lastpos . "\" text=\"" . specialchars(strip_tags($chantitle)) . "\" title=\"" . specialchars(strip_tags($chantitle)) . "\" description=\"" . specialchars($chandesc) . "\" xmlUrl=\"" . specialchars(strip_tags($xmlurl)) . "\" htmlUrl=\"" . specialchars(strip_tags($htmlurl)) . "\" refreshTime=\"" . $refreshtime . "\" showedItems=\"" . $showeditems . "\" isSubscribed=\"" . $issubscribed . "\" language=\"" . $chanlang . "\" />\n";
                    $i++;
                    $lastpos++;
                } elseif ($key == "xmlurl" . $i && $value != '' && $_POST["addbox" . $i] != 'checkbox') $i++;
            }
            $sfwrite.="\t</body>\n</opml>";
            echo "Added to subscription list (<b>".$zfcategory."</b>). Please go to subscriptions panel and customize.";
            mysql_query("UPDATE edp_newsopml SET opmlbody='".$sfwrite."'  WHERE opmlname='".$zfcategory."';");
    } else {
