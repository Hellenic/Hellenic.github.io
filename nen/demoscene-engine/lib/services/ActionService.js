...
(function (Service) {
    ...
    function resolveTargetMesh(action, object) {
        if (!action.targetModel) {
            if (object instanceof THREE.Mesh) return object;else if (object instanceof AreaTrigger || object instanceof Model) return object.getMesh();
        }
    ...
    function canTrigger(action) {
        if (!action.triggerTimes) return true;

        if (!action.timesTriggered) action.timesTriggered = 0;
