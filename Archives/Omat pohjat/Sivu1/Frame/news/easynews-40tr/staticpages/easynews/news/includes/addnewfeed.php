...
if (isset($_POST['subscribe']) && $_POST['subscribe'] == 'add to subscription list') {
    ...
    if ($opml_result == 'opmlok') {
        ...
        if ($opmlfilename) {
            ...
            $sfwrite.="\t\t<outline type=\"rss\" position=\"" . ($lastpos + 1) . "\" text=\"" . specialchars(strip_tags($chantitle)) . "\" title=\"" . specialchars(strip_tags($chantitle)) . "\" description=\"" . specialchars($chandesc) . "\" xmlUrl=\"" . specialchars(strip_tags($xmlurl)) . "\" htmlUrl=\"" . specialchars(strip_tags($htmlurl)) . "\" refreshTime=\"" . $refreshtime . "\" showedItems=\"" . $showeditems . "\" isSubscribed=\"" . $issubscribed . "\" language=\"" . $chanlang . "\" />\n";
            $sfwrite.="\t</body>\n</opml>";
            echo "Added to subscription list (<b>".$crtcategory."</b>)";
