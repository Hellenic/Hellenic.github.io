<?
  ...
  $ResultHtml=puHeading($Easy["Articles"]." ".$language['Search in']." ($FREET[$PageSection])",1)."<b>".$language['Search for']." \"$search\" </b><br><br>";
  ...
  switch ($Order) { case 1  : $QryOrder="t1.puHeading Asc"; break;
        ...
        $TotalNews=mysql_num_rows(puMyQuery("SELECT t1.ID,t1.puHeading, t1.puUserID, t1.puDate, t1.puTopic, t2.puScreenName FROM $FREETDB[$PageSection] as t1, edp_puusers as t2 WHERE t2.ID=t1.puUserID AND (t1.puHeading like '%$search%' OR t1.puBody like '%$search%' OR t1.puTopic like '%$search%'  OR t2.puScreenName like '%$search%') ORDER BY ".$QryOrder.";"));
        ...
        $TotalNews=mysql_num_rows(puMyQuery("SELECT  t1.ID, t1.puHeading, t1.puUserID, t1.puDate, t1.puTopic, t2.puScreenName FROM $FREETDB[$PageSection] as t1, edp_puusers as t2 WHERE  t1.puTopic='".$FREETDB[$PageSection]."' AND t2.ID=t1.puUserID  AND (t1.puHeading like '%$search%' OR t1.puBody like '%$search%'  OR t1.puTopic like '%$search%'  OR t2.puScreenName like '%$search%') ORDER BY ".$QryOrder.";"));
        $Newss=puMyQuery("SELECT t1.ID,t1.puHeading, t1.puUserID, t1.puDate, t1.puTopic, t2.puScreenName FROM $FREETDB[$PageSection]  as t1, edp_puusers as t2 WHERE t1.puTopic='".$FREETDB[$PageSection]."' AND   t2.ID=t1.puUserID AND (t1.puHeading like '%$search%' OR t1.puBody like '%$search%'  OR t1.puTopic like '%$search%'   OR t2.puScreenName like '%$search%')  ORDER BY ".$QryOrder." LIMIT $From, ".$Easy["head_per_page"].";");
        If ($TotalNews-$From-$Easy["head_per_page"]>0) { $More=TRUE; } else { $More=FALSE; }
        while ($News=mysql_fetch_array($Newss)) {
        $ResultHtml.="<a href='$EDP_SELF&page=individual&fage=$page&read=".$News["ID"]."&From=$From&Order=$Order&search=$search'>
